import { catalogueModules } from '@/lib/autresFormations'
import { HeroSection } from '@/components/ui/HeroSection'
import {
  autresFormationsCatalogueHero,
  autresFormationsParticipationCards,
} from '@/lib/content/pages/autresFormationsPageData'
import { AutresFormationsCatalogueClient } from './AutresFormationsCatalogueClient'

const Page = () => {
  const modules = catalogueModules.map((m) => ({
    slug: m.slug,
    titre: m.titre,
    imageSrc: m.imageSrc,
    imageAlt: m.imageAlt,
  }))

  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-primary-50">
      <HeroSection
        imageSrc={autresFormationsCatalogueHero.backgroundImage}
        imageAlt="Autres formations"
        subtitle={autresFormationsCatalogueHero.subtitle}
        title={autresFormationsCatalogueHero.title}
        description={autresFormationsCatalogueHero.description}
      >
        <div className="flex flex-wrap gap-4 items-center">
          {autresFormationsCatalogueHero.tags.map((tag) => (
            <span
              key={tag}
              className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-medium"
            >
              {tag}
            </span>
          ))}
        </div>
      </HeroSection>

      <AutresFormationsCatalogueClient modules={modules} />

      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-primary-900 mb-12 text-center">
            Modalités de participation
          </h2>
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {autresFormationsParticipationCards.map((item) => (
                <div
                  key={item.step}
                  className="flex gap-4 md:gap-6 items-start bg-primary-50 rounded-lg p-4 md:p-6 border border-primary-100 h-full min-w-0"
                >
                  <div className="flex-shrink-0 w-12 h-12 md:w-16 md:h-16 bg-primary-600 text-white rounded-full flex items-center justify-center font-bold text-base md:text-xl">
                    {item.step}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg md:text-xl font-semibold text-primary-900 mb-2">
                      {item.title}
                    </h3>
                    <p className="text-gray-600 leading-relaxed break-words text-sm md:text-base">
                      {item.description}
                    </p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      <section className="py-12 bg-gradient-to-r from-primary-600 to-secondary-600">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
              Intéressé par nos formations ?
            </h2>
            <a
              href="/contact"
              className="inline-block bg-white text-primary-600 font-semibold px-8 py-4 rounded-lg hover:bg-primary-50 transition-colors duration-200 shadow-lg hover:shadow-xl"
            >
              Demander une formation
            </a>
          </div>
        </div>
      </section>
    </div>
  )
}

export default Page
