...
export const ModuleImageCard: React.FC<ModuleImageCardProps> = ({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-primary-600/40 bg-gradient-to-br from-primary-900 to-primary-800 shadow-[0_22px_45px_rgba(2,6,23,0.45)]">
      <div className="relative h-52 w-full overflow-hidden md:h-64 lg:h-72">
        ...
        <span className="absolute top-3 left-4 flex h-10 w-10 items-center justify-center rounded-full bg-primary-600 text-sm font-semibold text-primary-50 shadow-lg ring-2 ring-white/20">
          {number}
        </span>
        <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primary-100/80">
              {label}
            </p>
            <h3 className="text-sm md:text-base font-semibold text-primary-50 leading-snug">
              {title}
            </h3>
          </div>
          {badge && (
            <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.18em] text-primary-50 ring-1 ring-white/25 backdrop-blur">
              {badge}
            </span>
          )}
        </div>
